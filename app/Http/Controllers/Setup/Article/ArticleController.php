...
namespace App\Http\Controllers\Setup\Article;
...
use App\Backend\Infrastructure\Forms\ArticleEntryRequest;
use App\Backend\Infrastructure\Forms\ArticleEditRequest;
...
class ArticleController extends Controller
{
    ...
    public function store(ArticleEntryRequest $request)
    {
        ...
        if($result['laravelStatusCode'] ==  ReturnMessage::OK){
            ...
            return redirect()->action('Setup\Article\ArticleController@index')
                ->with(FormatGenerator::message('Success', 'Article is created ...'));
        }
        else{
            return redirect()->action('Setup\Article\ArticleController@index')
                ->with(FormatGenerator::message('Fail', 'Article is not created ...'));
        }
    ...
    public function update(ArticleEditRequest $request){
        ...
        if($result['laravelStatusCode'] ==  ReturnMessage::OK){
            ...
            return redirect()->action('Setup\Article\ArticleController@index')
                ->with(FormatGenerator::message('Success', 'Article is updated ...'));
        }
        else{

            return redirect()->action('Setup\Article\ArticleController@index')
                ->with(FormatGenerator::message('Fail', 'Article is not updated ...'));
        }
    }
    ...
    public function check_article_name(){
        ...
        $article          = Article::where('country_id','=',$country_id)->where('article_name','=',$article_name)->whereNull('deleted_at')->get();
        $result        = false;
        if(count($article) == 0 ){
            $result = true;
        }
