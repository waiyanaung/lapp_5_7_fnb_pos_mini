...
class CountryController extends Controller
{
    ...
    {
        if(Auth::check()) {
            $countries      = $this->repo->getObjsAll();
    ...
    public function store(CountryEntryRequest $request)
    {
        ...
        $path         = base_path().'/public/images/country/';
        ...
        $paramObj->name         = $country_name;
        $paramObj->code         = $code;
        $paramObj->image_url    = '/images/country/' . $image_url_name;
        ...
        if($result['laravelStatusCode'] ==  ReturnMessage::OK){
            ...
            return redirect()->action('Setup\Country\CountryController@index')
                ->with(FormatGenerator::message('Success', 'Country is created ...'));
        }
        else{
            return redirect()->action('Setup\Country\CountryController@index')
                ->with(FormatGenerator::message('Fail', 'Country is not created ...'));
        }
    ...
    public function edit($id)
    {
        if(Auth::check()) {
            ...
            return view('backend.country.country')->with('obj', $country);
        }
    ...
    public function update(CountryEditRequest $request){
        ...
        if($result['laravelStatusCode'] ==  ReturnMessage::OK){
            ...
            return redirect()->action('Setup\Country\CountryController@index')
                ->with(FormatGenerator::message('Success', 'Country is updated ...'));
        }
        else{

            return redirect()->action('Setup\Country\CountryController@index')
                ->with(FormatGenerator::message('Fail', 'Country is not updated ...'));
        }
    }
    ...
    public function check_country_name(){
        ...
        $country          = Country::where('country_id','=',$country_id)->where('country_name','=',$country_name)->whereNull('deleted_at')->get();
        $result        = false;
        if(count($country) == 0 ){
            $result = true;
        }
