...
class CountryRepository implements CountryRepositoryInterface
{
    ...
    {
        $rawObjs = Country::whereNull('deleted_at')->get();
        $objs = array();
    ...
    public function delete($id)
    {
        ...
        try{
            ...
            //delete error log
            $date    = date("Y-m-d H:i:s");
            $message = '['. $date .'] '. 'error: ' . 'User '.$currentUser.' deleted  country_id = ' . $id . ' and got error -------'.$e->getMessage(). ' ----- line ' .$e->getLine(). ' ----- ' .$e->getFile(). PHP_EOL;
            LogCustom::create($date,$message);
        }
    }
    ...
    public function activate($id)
    {
        ...
        try{
            ...
            //activate info log
            $date = $tempObj->deleted_at;
            $message = '['. $date .'] '. 'info: ' . 'User '.$currentUser.' activated country_id = '.$tempObj->id . PHP_EOL;
            LogCustom::create($date,$message);
        }
        catch(\Exception $e){
            //activate error log
            $date    = date("Y-m-d H:i:s");
            $message = '['. $date .'] '. 'error: ' . 'User '.$currentUser.' deleactivatedted  country_id = ' . $id . ' and got error -------'.$e->getMessage(). ' ----- line ' .$e->getLine(). ' ----- ' .$e->getFile(). PHP_EOL;
            LogCustom::create($date,$message);
        }
    }
    ...
    public function getCountryByCountryId($country_id){
        // $result = DB::table('countries')->where('country_id', $country_id)->whereNull('deleted_at')->get();
        $result=Country::where('country_id', $country_id)->whereNull('deleted_at')->get();
        return $result;
    }

    public function checkToDelete($id){
        $result = DB::select("SELECT * FROM townships WHERE country_id = $id AND deleted_at IS NULL");
        return $result;
    }
