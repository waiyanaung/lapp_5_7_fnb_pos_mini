...
class TransactionOrderRepository implements TransactionOrderRepositoryInterface
{
    public function getObjs()
    {
        $rawObjs = TransactionOrder::whereNull('deleted_at')->get();
        $objs = array();
    ...
    {
        $rawObjs = TransactionOrder::whereNull('deleted_at')->orderBy('created_at', 'desc')->get();
        $objs = array();
        foreach($rawObjs as $rawObj){
            $objs[$rawObj->id] = $rawObj;
        }
    ...
    {
        $rawObjs = TransactionOrder::where('category_id',$category_id)->whereNull('deleted_at')->get();
        $objs = array();
        foreach($rawObjs as $rawObj){
            $objs[$rawObj->id] = $rawObj;
        }
    ...
    {
        $rawObjs = TransactionOrder::where('brand_id',$brand_id)->whereNull('deleted_at')->get();
        $objs = array();
        foreach($rawObjs as $rawObj){
            $objs[$rawObj->id] = $rawObj;
        }
    ...
    {
        $tbName = (new TransactionOrder())->getTable();
    ...
    public function create($paramObj)
    {
        ...
        try {
            ...
            $message = '['. $date .'] '. 'error: ' . 'User '.$currentUser.' created a TransactionOrder and got error -------'.$e->getMessage(). ' ----- line ' .$e->getLine(). ' ----- ' .$e->getFile(). PHP_EOL;
            LogCustom::create($date,$message);
    ...
    public function delete($id)
    {
        ...
        try{
            $tempObj = TransactionOrder::find($id);
